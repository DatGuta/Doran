...
namespace DR.Export.ReportExport.ReportOrderDetail {
    ...
    public class ReportOrderDetailExporter : OpentXmlBaseExport {
        private readonly ReportOrderDetail data;
        private readonly EExportFile exportFile;

        public ReportOrderDetailExporter(ReportOrderDetail data, EExportFile exportFile) : base() {
            this.data = data;
            this.exportFile = exportFile;
        }

        public static byte[] Export(ReportOrderDetail data, EExportFile exportFile) {
            return new ReportOrderDetailExporter(data, exportFile).Export();
        }
        ...
        public override byte[] Export() {
            ...
            try {
                ExportDSToExcel(data, filename, $"BÁO CÁO CHI TIẾT BÁN HÀNG\nTừ {data.From:dd/MM/yyyy} Đến {data.To:dd/MM/yyyy}");
        ...
        public void ExportDSToExcel(ReportOrderDetail data, string destination, string title) {
            ...
            using (var workbook = SpreadsheetDocument.Create(destination, SpreadsheetDocumentType.Workbook)) {
                ...
                //Insert Header
                var (columnNames, row) = GetHeader(table);
                sheetData.AppendChild(row);

                int[] numberStype = { 2, 2, 9, 2, 2, 2, 2, 2, 7, 7, 7 };
                ...
                foreach (DataRow dsrow in table.Rows) {
                    ...
                    foreach (var col in columnNames) {
                        ...
                        Cell cell = new Cell {
                            ...
                            DataType = style == 7 ? CellValues.Number : CellValues.String,
