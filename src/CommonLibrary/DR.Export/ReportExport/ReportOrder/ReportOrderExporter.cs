...
namespace DR.Export.ReportExport.ReportOrder {
    public class ReportOrderExporter : OpentXmlBaseExport {
        private readonly ReportOrderData data;
        private readonly EExportFile exportFile;

        public ReportOrderExporter(ReportOrderData data, EExportFile exportFile) : base() {
            this.data = data;
            this.exportFile = exportFile;
        }

        public static byte[] Export(ReportOrderData data, EExportFile exportFile) {
            return new ReportOrderExporter(data, exportFile).Export();
        }
        ...
        public override byte[] Export() {
            ...
            try {
                ExportDSToExcel(data, filename, $"BÁO CÁO KHÁCH HÀNG\n{data.From:dd/MM/yyyy} - {data.To:dd/MM/yyyy}");
        ...
        public void ExportDSToExcel(ReportOrderData data, string destination, string title) {
            ...
            using (var workbook = SpreadsheetDocument.Create(destination, SpreadsheetDocumentType.Workbook)) {
                ...
                uint sheetId = 1;
                var dataTable = data.Orders.ToDataTable();
                var ds = new DataSet();
                ...
                int[] numberStype = { 5, 2, 2, 2, 2, 2, 2, 2, 7, 7, 7, 7 };
                ...
                var startFooter = data.Orders.Count + 4;
                mergeCells.AppendChild(new MergeCell() { Reference = new StringValue($"A{startFooter}:I{startFooter}") });
                Row footerRow = GetFooter(data);
                sheetData.AppendChild(footerRow);

                //  Merge Duplicate Cell
                var customerCodes = data.Orders.Select(o => o.CustomerCode).Distinct().ToList();
        ...
        private Row GetFooter(ReportOrderData data) {
            ...
            Cell footerTotalBagCell = new Cell {
                CellValue = new CellValue(data.TotalBag),
                DataType = CellValues.Number,
                StyleIndex = 7,
                CellReference = $"J{startFooter}"
            };
            footerRow.AppendChild(footerTotalBagCell);
            Cell footerTotalKgCell = new Cell {
                CellValue = new CellValue(data.TotalKg),
                DataType = CellValues.Number,
                StyleIndex = 7,
                CellReference = $"K{startFooter}"
            };
            footerRow.AppendChild(footerTotalKgCell);
